"""
cdh.ingestion.boundaries
========================

Download administrative boundary polygons from the GeoBoundaries API and
return them as GeoDataFrames ready for spatial clipping.
...
def _fetch_names_cached(country_code: str, adm_level: int) -> list[dict[str, Any]]:
    """Download and cache only feature properties (no geometry)."""
...
def list_admin_units(
    ...
    adm_level : int
        Administrative level to query. Default: 2.
    name_column : str | None
        Column holding unit names. Auto-detected when None.
...
def get_country_boundary(country_code: str) -> gpd.GeoDataFrame:
    """Return the ADM0 (country-level) boundary as a GeoDataFrame.
    ...
    -------
    gpd.GeoDataFrame
        Country boundary in EPSG:4326. May be a MultiPolygon (islands, etc.).
    """
    gdf = _fetch_geojson_cached(country_code, adm_level=0)
...
def get_admin_boundary(
    ...
    adm_level : int
        Administrative level. Default: 2.
    name_column : str | None
    ...
    ------
    ValueError
        If no feature matches or the match is ambiguous.
    """
